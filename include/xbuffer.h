...
void  xbuf_frfile  (xbuf_t *ctx, const char *szFile);

// save the buffer in a file
int  xbuf_tofile  (xbuf_t *ctx, const char *szFile);
...
void  xbuf_attach  (xbuf_t *ctx, const char *ptr, s32 size, s32 len);
...
char *xbuf_ncat    (xbuf_t *ctx, const char *src, s32 srclen);

// copy the string 'str' into the buffer
char *xbuf_cat     (xbuf_t *ctx, const char *src);

// format 'a la sprintf()' into the buffer
char *xbuf_xcat    (xbuf_t *ctx, const char *src, ...);
...
char *xbuf_findstr (xbuf_t *ctx, const char *str);

// replace all occurences of the 'old' string by the 'new' string in the buffer
char *xbuf_repl    (xbuf_t *ctx, const char *old, const char *newstr);
...
s32   xbuf_http    (xbuf_t *ctx, s32 code, const char *body);
...
//           "&arg1=val1&arg2=val2");
#define HTTPS 100
s32   xbuf_frurl   (xbuf_t *ctx, const char *host, u32 port, u32 method, 
                    const char *uri, u32 mstimeout, const char *headers);
