...
typedef struct // customizable connection context
{
   ip_t   ip;           // "a.b.c.d" + IP address as u32 (see above)
   int    fd;           // client socket
   u32    id       :20, // 0-1m+ conn_t id (id == fd if fd is not closed)
          protocol : 7, // 0-128 entry in PROTOCOL_def enum {}
...
//
int run_cmd(const char *cmd, int argc, const char *argv[]);
...
// HTML escaping
// ----------------------------------------------------------------------------
u32  url_encode   (void *dst, const void *src, u32 maxdstlen);  // return len
u32  escape_html  (void *dst, const void *src, u32 maxdstlen);  // return len

u32  unescape_html(void *str); // return len
int  html2txt     (const void *html, void *text, int maxtxlen); // return len
