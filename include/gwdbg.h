...
{
   int(*p_proto_used)(char *name, int idx) =
   (int(*)(char *name, int idx))
   _proto_used;
   return p_proto_used(name, idx);
}

int proto_2idx(const char *name)
{
   int(*p_proto_2idx)(char *name) =
   (int(*)(char *name))
   _proto_2idx;
   return p_proto_2idx(name);
}

char *proto_2name(const u32 i)
{
   char*(*p_proto_2name)(const u32 i) =
   (char*(*)(const u32 i))
   _proto_2name;
   return p_proto_2name(i);
}
...
extern u32 url_encode(u8 *dst, const u8 *src, u32 maxdstlen)
{
   u32(*p_url_encode)(u8 *dst, const u8 *src, u32 maxdstlen) =
   (u32(*)(u8 *dst, const u8 *src, u32 maxdstlen))
   _url_encode;
   return p_url_encode(dst, src, maxdstlen);
}

extern u32 escape_html(u8 *dst, const u8 *src, u32 maxdstlen)
{
   u32(*p_escape_html)(u8 *dst, const u8 *src, u32 maxdstlen) =
   (u32(*)(u8 *dst, const u8 *src, u32 maxdstlen))
   _escape_html;
...
extern int sendemail(char *mail_server, char *src_mailaddr, char **dst_mailaddr,
         char *subject, char *text, int text_len, char *auth_user, 
         char *auth_pass, int auth_meth, char *error)
{
   int(*p_sendemail)(char *mail_server, char *src_mailaddr, char **dst_mailaddr,
         char *subject, char *text, int text_len, char *auth_user, 
         char *auth_pass, int auth_meth, char *error) =
   (int(*)(char *mail_server, char *src_mailaddr, char **dst_mailaddr,
         char *subject, char *text, int text_len, char *auth_user, 
         char *auth_pass, int auth_meth, char *error))
   _sendemail;
   return p_sendemail(mail_server, src_mailaddr, dst_mailaddr, subject, text,
                      text_len, auth_user, auth_pass, auth_meth, error);
}
...
extern int run_cmd(const char *cmd, int argc, const char *argv[])
{
   int(*p_run_cmd)(const char *cmd, int argc, const char *argv[]) =
   (int(*)(const char *cmd, int argc, const char *argv[]))
   _run_cmd;
